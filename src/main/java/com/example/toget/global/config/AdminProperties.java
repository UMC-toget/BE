...
/**
 * 관리자 계정 식별 설정 — application.yaml의 admin.* 값을 바인딩한다.
 *
 * <p>[설계 배경] users 테이블에 role 컬럼을 추가하지 않고, 고정 운영되는 관리자 계정
 * 1개의 식별자만 환경변수로 관리한다. 스키마 변경 없이 즉시 적용할 수 있고,
 * 운영/개발 환경별로 다른 계정을 지정할 수 있다.
 *
 ...
 *
 * @param email    관리자 계정의 이메일. 비어 있으면 관리자 API를 전면 차단한다(fail-closed).
 * @param provider 관리자 계정의 소셜 로그인 공급자. 미설정 시 GOOGLE.
 */
@ConfigurationProperties(prefix = "admin")
public record AdminProperties(String email, OAuthProvider provider) {
