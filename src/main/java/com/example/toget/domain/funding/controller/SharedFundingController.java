    ...
    @Operation(
            ...
            description = """
                    ...
                    **`status`** 는 MY_GIFT 기준 `SETTLING`(진행 중) / `ENDED`(마감) 두 값만 내려갑니다. \
                    개설자가 종료일 전에도 직접 마감할 수 있고, 종료일이 지나면 매일 00:05(KST) 배치로 \
                    자동으로도 마감(ENDED)됩니다. 다만 배치 실행 전까지 최대 하루 정도 지연이 있을 수 있으니, \
                    `endDate`만 보고 마감 여부를 판단하지 말고 이 `status` 필드를 기준으로 판단하세요.
