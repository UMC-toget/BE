...
class FundingAutoEndServiceTest {
    ...
    void autoEndsExpiredSettlingFundings() {
        ...
        Funding funding1 = Funding.createMyGift(1L, 1L, "제목1", "홍길동",
                LocalDate.now(), LocalDate.now().minusDays(10), LocalDate.now().minusDays(1),
                null, null, 100_000L);
        Funding funding2 = Funding.createMyGift(2L, 2L, "제목2", "김철수",
                LocalDate.now(), LocalDate.now().minusDays(20), LocalDate.now().minusDays(5),
                null, null, 50_000L);
    ...
    @Test
    @DisplayName("MY_GIFT + SETTLING + 오늘 날짜 기준으로만 조회한다 (TOGETHER_GIFT/다른 상태는 대상이 아님)")
