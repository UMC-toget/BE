...
class FundingTogetherDraftServiceTest {

    @Mock
    private FundingTogetherDraftRepository fundingTogetherDraftRepository;

    @Mock
    private ActiveUserReader activeUserReader;

    @InjectMocks
    private FundingTogetherDraftService fundingTogetherDraftService;

    @Nested
    @DisplayName("함께 선물 임시 저장 삭제")
    class DeleteDraft {

        @Test
        @DisplayName("본인의 임시 저장 드래프트를 성공적으로 삭제한다")
        void deleteSuccess() {
            Long userId = 1L;
            Long draftId = 10L;
            User activeUser = mock(User.class);
            FundingTogetherDraft draft = FundingTogetherDraft.builder()
                    .id(draftId)
                    .userId(userId)
                    .build();

            given(activeUserReader.getActiveUser(userId)).willReturn(activeUser);
            given(fundingTogetherDraftRepository.findById(draftId)).willReturn(Optional.of(draft));

            fundingTogetherDraftService.delete(userId, draftId);

            verify(fundingTogetherDraftRepository).delete(draft);
        }
        ...
        void deleteNotFound() {
            ...
            given(activeUserReader.getActiveUser(userId)).willReturn(activeUser);
            given(fundingTogetherDraftRepository.findById(draftId)).willReturn(Optional.empty());

            assertThatThrownBy(() -> fundingTogetherDraftService.delete(userId, draftId))
